...
> In AI-assisted development, the "compiler" represents the AI model or system that generates code from prompts. Instead of manually refining or beautifying generated code—a time-consuming and error-prone process—focus on fine-tuning the AI itself to produce higher-quality output directly. Fine-tuning involves training the model on specific datasets or prompts to improve its accuracy, consistency, and alignment with your project's needs. This approach leverages the principle that "duplication is good" by ensuring the AI generates coherent, patterns consistently across the codebase, while avoiding the pitfalls of manual polishing, which can introduce inconsistencies and divert attention from core functionality. By investing in the AI's capabilities, you create a more efficient and reliable workflow where code is correct by construction, reducing the need for post-generation tweaks and upholding the tenet that "runnable is better than beautiful."

### 19. Never commit compiler output to your codebase.
> Compiler output—such as binaries, object files, or any automatically generated artifacts—is derived from the source code and should be excluded from version control. These files are not part of the human-readable source and can be reliably reproduced through the build process. Committing them leads to repository bloat, potential merge conflicts, and obscures the true history of changes. While "duplication is good" in the context of AI-generated code patterns for maintaining architectural coherence, it does not justify storing compiler output, which represents redundant and transient data. Instead, focus on keeping the codebase lean with only essential sources, configurations, and prompts, ensuring clarity and efficiency in both human and AI collaboration. This aligns with the principle that "simple is better than complex," as a clean codebase simplifies maintenance and reduces cognitive overhead.

### 20. Thinking mode is not a waste of time. Do not rush to fix your prompts under instruct mode.
> In AI-assisted development, "Thinking mode" represents a deliberate, reflective state where you analyze problems, consider design trade-offs, and refine your understanding before issuing prompts. This process is crucial for creating effective, well-structured prompts that guide AI to generate correct and efficient code. Rushing to tweak prompts in "instruct mode"—where you focus on immediate, iterative fixes—often leads to superficial changes that miss underlying issues, resulting in fragmented code and wasted effort. By embracing thoughtful analysis, you align with the principle that "explicit is better than implicit, but verified is better than explicit," ensuring that your prompts are thoroughly considered and verified through mental modeling before execution. This approach fosters robust solutions and reduces the need for repetitive debugging, ultimately saving time and enhancing code quality.

### 21. Without your manager, everything works smoothly; do not micromanage.
> In AI-assisted development, the AI assistant functions best when given autonomy within clear boundaries. Constant human intervention—such as overly detailed prompts, frequent corrections, or step-by-step directives—disrupts the AI's natural pattern-matching and code generation capabilities, leading to fragmented output and reduced efficiency. By trusting the AI to handle implementation details after providing well-defined goals, developers enable a smoother workflow that aligns with the core principles of Vibe Coding. This approach emphasizes that "Simple is better than complex," as it minimizes unnecessary complexity introduced by micromanagement and allows the AI to produce coherent, runnable code more effectively. Just as a skilled manager empowers their team, a developer should guide the AI with high-level intent rather than controlling every detail, fostering a collaborative and productive environment.
